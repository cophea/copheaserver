...
class RegisterModel extends Model {
	...
	public function isValidName($name, $old_name = null) {
		...
		$site_config = D('Xdata')->get('admin_Config:site');
		...
		if(in_array($name, $protected_name)) {
			$this->_error = '抱歉，该昵称不允许被使用';				// 抱歉，该昵称不允许被使用
			$res = false;
			return $res;
		}
		if(!$res) {
			$this->_error = '昵称长度必须在2-10个汉字之间';			// 昵称长度必须在2-10个汉字之间
			return $res;
		}
		if(($name != $old_name) && $this->_user_model->where('`uname`="'.mysql_escape_string($name).'"')->find()) {
			$this->_error = '该用户名已被使用';				// 该用户名已被使用
			$res = false;
		}
		// 敏感词
	...
	public function isValidEmail_invite($email, $old_email = null) {
		...
		if(!$res) {
			$this->_error = '无效的邮箱地址';			// 无效的Email地址
	...
	public function isValidPassword($pwd, $repwd) {
		...
		$length = strlen($pwd);
		if($length < 6) {
			$this->_error = '密码太短了，最少6位';			// 密码太短了，最少6位
			$res = false;
		} else if ($length > 15) {
			$this->_error = '密码太长了，最多15位';
			$res = false;
		} else if($pwd !== $repwd) {
			$this->_error = '新密码与确认密码不一致';		// 新密码与确认密码不一致
			$res = false;
		}
	...
	public function sendActivationEmail($uid, $node ='register_active') {
		...
		if(!$user_info) {
			...
			//D('Notify')->sendNotify($uid, $node, $config);
