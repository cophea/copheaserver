...
class LogsModel extends Model {
	...
	protected $fields = array('id','uid','uname','app_name','group','action','data','ctime','url','isAdmin','ip','keyword','corp_id');
	...
	public function getBydate($dateFrom,$dateTo){
		$table = $table ? $this->tablePrefix.'x_logs_'.$table : $this->tablePrefix.'x_logs';
		//$map['mtime'] = array('between',array($dateFrom,$dateTo));
		$dateFrom = strtotime($dateFrom." 00:00:00");
		$dateTo = strtotime($dateTo." 23:59:59");
		$map['ctime'] = array(array('gt',$dateFrom),array('lt',$dateTo));
		$map['corp_id'] = $_SESSION['cid'];
		$list = $this->table($table)->where($map)->order('id DESC')->findAll();
		foreach($list as $key => $v) {
			$tempData = $this->__paseTemplate($v);
			$list[$key]['data'] = $tempData['data'];
			$list[$key]['type_info'] = $tempData['info'];
